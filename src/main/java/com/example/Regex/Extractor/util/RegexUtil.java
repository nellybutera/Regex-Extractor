...
public class RegexUtil {
    ...
    private static final String PHONE_REGEX = "\\+?\\d[\\d\\s().-]{6,}\\d";
    private static final String CREDIT_CARD_REGEX = "\\b(?:\\d{4}[-\\s]?){3}\\d{4}\\b";
    private static final String CURRENCY_REGEX = "(?:\\$|€|£)?\\d+(?:,\\d{3})*(?:\\.\\d{1,2})?";
    ...
    public static Map<String, List<String>> extractAll(String text) {
        ...
        result.put("emails", extractMatches(text, EMAIL_REGEX));
        result.put("urls", extractMatches(text, URL_REGEX));
        result.put("phoneNumbers", extractMatches(text, PHONE_REGEX));
        result.put("creditCards", extractMatches(text, CREDIT_CARD_REGEX));
        result.put("currencies", extractMatches(text, CURRENCY_REGEX));
        result.put("times", extractMatches(text, TIME_REGEX));
        result.put("htmlTags", extractMatches(text, HTML_TAG_REGEX));
        result.put("hashtags", extractMatches(text, HASHTAG_REGEX));
